...
class message_label extends rcube_plugin
{
    ...
    public function init() {
        ...
        if(isset ($_SESSION['user_id'])) {
            ...
            if($rcmail->action == 'show') {
                ...
                $this->add_hook('message_load', array($this, 'init_message_flag'));
            }

            if(($rcmail->action == '' || $rcmail->action == 'show') && $rcmail->task == 'mail') {
                $this->add_hook('template_object_mailboxlist', array($this, 'folder_list_label'));
                $this->register_handler('label_list_menu', array($this, 'render_labels_menu'));
                $labellink = html::a(array( 'href'=>'?_action=label_preferences&_task=settings', 'class' => 'active'),$this->gettext('label_pref'));
                $this->api->add_content(html::tag('div', array('class' => 'bottom-left-inbox'), $labellink), 'mailboxoptions');
            }
     ...
     *
     */
    function message_label_imap_set($p) {
        ...
        if(($uids = get_input_value('_uid', RCUBE_INPUT_POST)) && ($flag = get_input_value('_flag', RCUBE_INPUT_POST))) {
            ...
            if($_POST['_from'] == 'show') {
                ...
                $this->rc->output->command('show_message', $_POST[_uid]);
            } else {
    ...
    function message_label_search() {
        ...
        if($use_saved_list && $_SESSION['all_folder_search']['uid_mboxes'])
            $result_h = $this->get_search_result(); else
        ...
        if(($uids = get_input_value('_uid', RCUBE_INPUT_POST)) && ($flag = get_input_value('_flag', RCUBE_INPUT_POST))) {
            ...
            if($flag == 'SEEN' || $flag == 'UNSEEN' || ($flag == 'DELETED' && !$skip_deleted)) {
                ...
                foreach ($mbox_names as $mbox)
                    $this->rc->output->command('set_unread_count', $mbox, $this->rc->imap->count($mbox, 'UNSEEN'), ($mbox == 'INBOX'));
            } else if($flag == 'DELETED' && $skip_deleted) {
                if($_POST['_from'] == 'show') {
                    if($next = get_input_value('_next_uid', RCUBE_INPUT_GPC))
                        $this->rc->output->command('show_message', $next); else
    ...
    public function message_label_redirect() {
        $task = get_input_value('_task', RCUBE_INPUT_POST);
        $action = get_input_value('_action', RCUBE_INPUT_POST);
        $search_id = get_input_value('search_id', RCUBE_INPUT_POST);
        $p = array('_task' => ($task) ? $task : 'settings', '_action' => ($action) ? $action : ' ', 'search_id' => ($search_id) ? $search_id : ' ');
        $this->rc->output->redirect($p);
    }

    /**
     * render labbel menu for markmessagemenu
     */
    public function render_labels_menu() {
        $prefs = $this->rc->config->get('message_label', array());
        if(count($prefs) > 0) {
            $attrib['class'] = ' ';
            ...
            foreach ($prefs as $key => $p) {
                $first = !$key ? 'firstLilink' : '';
                $ul .= html::tag('li', array('class' => $first), html::a(array('style' => 'background-color:' . $p['color'], 'href' => '#', 'onclick' => 'rcmail.label_messages(\'' . $p['id'] . '\')'), $p['text']));
            }
    ...
    function folder_list_label($args) {
        ...
        if($display_label == 'false')
           ;// $style = 'display: none;'; //@todo реализовать поддержку скрытия меню ярлыков в левой колонке

        if(count($prefs) > 0) {
            $list = "";
            /* <li class="mailbox inspiration"><a href="#">Inspiration</a></li> */
            foreach ($prefs as $p) {
                $link = '';

                $link = html::tag('span', array('class' => 'linkMask', 'style' => 'background-color:' . $p['color']), '');
                $link .= html::a(array('href' => '#'), $p['text']);
                $list .= html::tag('li', array('class' => 'links ', 'id' => 'rcmrow' . $p['id']), $link);
            }
            $args['content'] .= html::div(array('class' => 'lmenu', 'id' => 'drop_labels', 'style' => $style), html::tag('ul', array('id' => 'labellist'), $list));
        } else {
            $args['content'] .= html::div('lmenu',
                    html::a(array('href' => '?_action=label_preferences&_task=settings',
                        //'onclick' => 'return rcmail.command(\'plugin.label_redirect\',\'true\',true)'
                        ),
                        $this->gettext('label_create')));
        }
    ...
    function label_preferences($args) {
        if($args['section'] == 'label_preferences') {
            ...
            //insert empty row
            $args['blocks']['create_label']['options'][$i++] = array( 'content' => $this->get_form_row());

            foreach ($prefs as $p) {
                $args['blocks']['list_label']['options'][$i++] = array( 'content' => $this->get_form_row($p['id'], $p['header'], $p['folder'], $p['input'], $p['color'], $p['text'], true));
            }
        }
        return ($args);
    }
    ...
    function preferences_section_list($args) {
        $args['list']['label_preferences'] = array('id' => 'label_preferences', 'section' => Q($this->gettext('label_title')));
        return ($args);
    }
    ...
    function init_message_flag() {
        if(!empty ($_GET['_uid'])) {
            $uid = get_input_value('_uid', RCUBE_INPUT_GET);
            $params = $this->rc->storage->get_message($uid);

            if($params->flags['FLAGGED'] == true)
                $this->rc->output->set_env('flagged', true); else
                $this->rc->output->set_env('flagged', false);

            $prefs = $this->rc->config->get('message_label', array());

            foreach ($prefs as $key => $val)
                $real_prefs[strtoupper($val['id'])] = $val;

            $labels_key = array();
            foreach ($params->flags as $k => $val) {

                if(preg_match("#^LABELS_[0-9A-z]*$#", $k)) {
                    $labels_key[] = str_ireplace("LABELS_", '', $k);
                }
            }

            $real_labels = array();

            foreach ($labels_key as $key) {
                $real_labels[$key] = $real_prefs[$key];
            }
            if(count($real_labels)) {
                $html = '';
                foreach ($real_labels as $k => $v) {
                    if(empty ($v['text']))
                        $v['text'] = 'label';
                    $k = 'label' . strtolower($k);
                    $html .= html::span(array('class' => 'linkedMessage', 'style' => "background-color:{$v['color']};", 'id' => "$k"), $v['text']) . "&nbsp;";
                }
                $this->api->add_content($html, 'message_label');
            }
        }
    }
    ...
    private function perform_search($search_string, $folders, $label_id) {
        ...
        if($folders[0] == 'all' || empty ($folders))
            $folders_search = $this->rc->imap->list_folders_subscribed(); else
        ...
        for ($x = 0; $x < count($result_h); $x++) {
            for ($y = 0; $y < count($result_h); $y++) {
                ...
                switch ($_SESSION['sort_col']) {
                    case 'date':
                        $first = $result_h[$x]->timestamp;
                        $second = $result_h[$y]->timestamp;
                        break;
                    case 'size':
                        $first = $result_h[$x]->size;
                        $second = $result_h[$y]->size;
                        break;
                    case 'subject':
                        $first = strtolower($result_h[$x]->subject);
                        $second = strtolower($result_h[$y]->subject);
                        break;
                    case 'from':
                        $first = strtolower($result_h[$x]->from);
    ...
    private function get_form_row($id = '', $header = 'from', $folder = 'all', $input = '', $color = '#000000', $text = '', $delete = false) {
        ...
        if(!$delete) {
            $button = html::a(array('href' => '#cc', 'class' => 'lhref add_label', 'onclick' => 'return rcmail.command(\'save\',\'\',this)'), $this->gettext('add_row'));
        } else {
            $button = html::a(array('href' => '#cc', 'class' => 'lhref delete_label', 'onclick' => 'return rcmail.command(\'plugin.label_delete_row\',\'' . $id . '\')'), $this->gettext('delete_row'));
        }

        $content = $select_color . "&nbsp;" . $text . "&nbsp;" . $header_select->show($header) . "&nbsp;" . $this->gettext('label_matches') . "&nbsp;" . $input->show() . "&nbsp;" . $id_field . "&nbsp;" . $this->gettext('label_folder') . "&nbsp;" . str_replace("&amp;nbsp;", "&nbsp;", $folder_search->show($folder)) . "&nbsp;" . $button;

        return html::span(array('class'=>'label_list_form'), $content);
    }
}
