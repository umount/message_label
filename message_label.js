...
rcube_webmail.prototype.unlabel_message = function(label) {

    if (this.env.action == 'show') {
        var ids = [];
        ids.push(rcmail.env.uid);
        this.toggle_label_status('un' + label, ids, 'label', false);
    }
}
...
rcube_webmail.prototype.toggle_label_status = function(flag, a_uids, type, update) {
    ...
    var i, len = a_uids.length,
            url = '_uid=' + rcmail.uids_to_list(a_uids) + '&_flag=' + flag + '&_type=' + type + '&_from=' + rcmail.env.action;
...
rcube_webmail.prototype.redirect_label_pref = function(post) {
    if (post != null) {
        rcmail.http_post('plugin.message_label_redirect', '_action=label_preferences&_task=settings');
    }
}
...
function show_label_list(event) {
    /*
     if($('#popupmenu4:visible').length){
...
rcube_webmail.prototype.labels_select = function(list) {
    ...
    if (uid) {
        if (this.env.action == 'show') {
            ...
            rcmail.http_post('plugin.message_label_redirect', '_task=mail&search_id=' + uid);
            return false;
            ...
            rcmail.clear_message_list();
            this.http_post('plugin.message_label_search', post, lock);
        }
...
if (window.rcmail) {
    rcmail.register_command('plugin.label_list', 'show_label_list', false);
    ...
    rcmail.addEventListener('init', function(evt) {
        ...
        rcmail.register_button('plugin.label_list', 'label_list', '', 'moreActions topLink');

        if (rcmail.gui_objects.labellist && (rcmail.env.action == "" || rcmail.env.action == 'show')) {
            var p = rcmail;
            ...
            //reload init
            rcmail.label_list.init = function() {
            ...
            if (rcmail.env.action != 'show') {
                ...
                rcmail.message_list.addEventListener('select', function(o) {
                    ...
                    if (rcmail.message_list.selection.length)
                        rcmail.enable_command('plugin.label_list', true);
                    else
                        rcmail.enable_command('plugin.label_list', false);
    ...
    rcmail.addEventListener('beforelist', function() {
        if (rcmail.env.no_search != 1 && location.search.search(/_search_id=/) != -1) {
            var match = location.search.match(/_search_id=([a-z0-9]+)/)
            rcmail.labels_select(match[1]);
            $('#rcmrow' + match[1]).addClass('selected');
            rcmail.env.no_search = 1;
            return false;
        }
...
$(document).ready(function() {
    ...
    $('.linkedMessage')
        ...
        rcmail.unlabel_message(id);
    })
    $('#popupmenu4 a').click(function() {
        $('#popupmenu4').hide();
    })
